function getDataForGraphic() {
    ...
    let n = data.length;
    for (let i = 0; i < n; i++) {
        if (!Object.keys(dataForGraph).includes(data[i][5])) {
            dataForGraph[data[i][5]] = [];
        } else {
            dataForGraph[data[i][5]].push(data[i][6]);
        }
    }
    return dataForGraph;
}
...
function createGraphic(ox, oy) {
    ...
    let height = 500, width = 500, margin = 30;
